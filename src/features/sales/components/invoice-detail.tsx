...
import { useInvoiceHistory, type InvoiceHistoryAction } from "@/hooks/useInvoiceHistory";
...
export function InvoiceDetail({
  ...
  const { history, isLoading: historyLoading } = useInvoiceHistory(invoice?.id ?? null, "sale");
          ...
          <div className="grid grid-cols-3 gap-6 mb-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span
                  className={cn(
                    invoice.status === "returned" ? "text-error font-medium" : "text-slate-900"
                  )}
                >
...
function HistorySection({
  ...
  const hasDetails = (entry: (typeof history)[0]) => {
    return (entry.oldValues && Object.keys(entry.oldValues).length > 0) ||
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {history.map((entry) => {
              ...
              return (
                <div key={entry.id} className="border border-slate-100 rounded-lg">
                  <button
                    type="button"
                  ...
                  >
                    <div
                      className={cn(
                        ...
                        entry.action === "payment_recorded" && "bg-success-light",
                        entry.action === "deleted" && "bg-error-light"
                      )}
                    >
                      <ActionIcon
                        className={cn("h-4 w-4", actionConfig?.color ?? "text-slate-500")}
                      />
                    </div>
                                      ...
                                      <div>
                                        ...
                                        <ul className="ml-4 mt-1 space-y-0.5">
                                          {formatted.map((item, idx) => (
                                            <li key={idx} className="text-slate-900">• {item}</li>
                                          ))}
                                        </ul>
                                      </div>
                                    ) : (
