...
const calculateItemTotal = (quantity: number, unitPrice: number, discountPercent: number, taxPercent: number) => {
...
const calculateInvoiceTotal = (items: { quantity: number, unitPrice: number, discountPercent: number, taxPercent: number }[], invoiceDiscountValue = 0, discountType: 'percent' | 'fixed' = 'fixed') => {
...
describe("Invoice Calculations (Strict Rounding)", () => {
    ...
    it("should match the user reported case", () => {
        ...
        const item1 = { quantity: 10, unitPrice: 19.99, discountPercent: 0, taxPercent: 10 }; 
        const item2 = { quantity: 5, unitPrice: 5.55, discountPercent: 0, taxPercent: 10 };
        const item3 = { quantity: 1, unitPrice: 100.01, discountPercent: 0, taxPercent: 10 };
    ...
    it("should handle exact rounding for tax", () => {
        // 100 * 0.18 = 18
        const item = { quantity: 1, unitPrice: 100, discountPercent: 0, taxPercent: 18 };
        const res = calculateInvoiceTotal([item]);
        expect(res.taxAmount).toBe(18.00);
        expect(res.total).toBe(118.00);
    });

    it("should round tax correctly (rounding up 0.005)", () => {
        // 1.5 * 0.05 = 0.075 -> rounds to 0.08
        const item = { quantity: 1, unitPrice: 1.5, discountPercent: 0, taxPercent: 5 };
        const res = calculateInvoiceTotal([item]);
    ...
    it("should match sum of line items exactly", () => {
        ...
        const item1 = { quantity: 1, unitPrice: 10.33, discountPercent: 0, taxPercent: 0 };
        const item2 = { quantity: 1, unitPrice: 10.33, discountPercent: 0, taxPercent: 0 };
        const item3 = { quantity: 1, unitPrice: 10.34, discountPercent: 0, taxPercent: 0 };
        
        // Sum = 31.00
        const res = calculateInvoiceTotal([item1, item2, item3]);
        expect(res.subtotal).toBe(31.00);
    });
});
