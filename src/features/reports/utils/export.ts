...
const saveFile = async (filename: string, content: Blob | Uint8Array, extension: string) => {
    ...
    if (isTauri) {
        try {
            ...
            if (errorMsg.includes("os error 32") || errorMsg.includes("used by another process")) {
                alert("File is currently open in another application. Please close the file and try again.");
                return; 
            }
...
const downloadInBrowser = (filename: string, content: Blob | Uint8Array, extension: string) => {
...
export const exportToExcel = <T>(
  ...
  saveFile(filename, new Uint8Array(wbout), "xlsx");
};
...
export const exportToPdf = <T>(
    ...
    content: [
      { text: title, style: "header", margin: [0, 0, 0, 10] },
      {
        ...
        layout: {
            ...
            paddingTop: (i: number) => 4,
            paddingBottom: (i: number) => 4,
        },
      },
      { text: `Generated on ${new Date().toLocaleDateString()}`, style: 'footer', margin: [0, 10, 0, 0], alignment: 'right', fontSize: 7, color: '#6b7280' }
    ] as any,
    styles: {
      header: {
        fontSize: 16,
  ...
  pdfDocGenerator.getBuffer((buffer) => {
      saveFile(filename, new Uint8Array(buffer), "pdf");
  });
};
...
export const printReport = <T>(
  ...
  title: string
   ...
   const headers = columns.map(c => `<th style="text-align: left; padding: 8px; border-bottom: 2px solid #e2e8f0;">${c.label}</th>`).join("");
   
   const rows = data.map(item => {
       const cells = columns.map(col => {
           const val = col.format ? col.format(item[col.key], item) : item[col.key];
           return `<td style="padding: 8px; border-bottom: 1px solid #e2e8f0;">${val ?? ""}</td>`;
       }).join("");
       return `<tr>${cells}</tr>`;
   }).join("");
