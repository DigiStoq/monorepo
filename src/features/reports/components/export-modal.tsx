...
export function ExportModal<T>({
  ...
  title = "Export Data",
  filename = "export",
}: ExportModalProps<T>): React.ReactNode {
  ...
  const toggleColumn = (key: keyof T): void => {
  ...
  const handleExportCsv = async (): Promise<void> => {
    await exportToCsv(data, selectedColumns, filename);
    onClose();
  };

  const handleExportExcel = async (): Promise<void> => {
    await exportToExcel(data, selectedColumns, filename);
    onClose();
  };

  const handleExportPdf = async (): Promise<void> => {
    await exportToPdf(data, selectedColumns, title, filename);
    onClose();
  };
