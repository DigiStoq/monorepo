...
import { Card, CardBody, Badge, Select, type SelectOption, Input } from "@/components/ui";
...
export function DayBookReport() {
  ...
  const { entries: dayBookEntries, totals, isLoading } = useDayBookReport(selectedDate);
  ...
  const filteredTotals = useMemo(() => {
    if (typeFilter === "all") {
      return { debit: totals.debit, credit: totals.credit, net: totals.debit - totals.credit };
    }
          ...
          <CardBody className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{formatCurrency(filteredTotals.debit)}</p>
              </div>
          ...
          <CardBody className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{formatCurrency(filteredTotals.credit)}</p>
              </div>
          ...
          <CardBody className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={cn("text-2xl font-bold", filteredTotals.net >= 0 ? "text-success" : "text-error")}>
                  {formatCurrency(Math.abs(filteredTotals.net))}
                  <span className="text-sm ml-1">{filteredTotals.net >= 0 ? "Dr" : "Cr"}</span>
                </p>
              </div>
            </div>
          </CardBody>
        ...
        <CardBody className="p-0">
          {filteredEntries.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <p className="text-slate-500">No transactions recorded for this date</p>
            </div>
          ) : (
            <table className="w-full">
              ...
              <tfoot>
                <tr className="bg-slate-50 font-medium">
                  ...
                  <td className="px-4 py-3 text-right text-blue-600">{formatCurrency(filteredTotals.debit)}</td>
                  <td className="px-4 py-3 text-right text-green-600">{formatCurrency(filteredTotals.credit)}</td>
                </tr>
              </tfoot>
            </table>
          )}
