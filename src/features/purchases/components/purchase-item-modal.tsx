...
// Matches the internal LineItem type in PurchaseInvoiceForm
...
interface PurchaseItemModalProps {
  ...
  item?: PurchaseLineItem; // If editing
...
export function PurchaseItemModal({
  ...
  defaultTaxRate = 0,
}: PurchaseItemModalProps): React.ReactNode {
  // State
  ...
  const handleItemChange = (newItemId: string): void => {
    ...
    if (selected) {
      setUnitPrice(selected.purchasePrice);
  ...
  const itemOptions: SelectOption[] = useMemo(
    () => [
      ...
      ...items.map((i) => ({
        value: i.id,
        label: `${i.name} - ${formatCurrency(i.purchasePrice)}`,
      })),
    ],
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={item ? "Edit Purchase Item" : "Add Purchase Item"}
      size="md"
      footer={
        <>
