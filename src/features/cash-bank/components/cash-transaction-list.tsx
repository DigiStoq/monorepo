...
import { Card, CardBody, Input, Select, type SelectOption } from "@/components/ui";
import {
  Search,
  Wallet,
  ArrowUpCircle,
...
const typeConfig: Record<CashTransaction["type"], { label: string; icon: React.ReactNode; color: string }> = {
  in: { label: "Cash In", icon: <ArrowUpCircle className="h-4 w-4" />, color: "text-success" },
  out: { label: "Cash Out", icon: <ArrowDownCircle className="h-4 w-4" />, color: "text-error" },
  adjustment: { label: "Adjustment", icon: <Settings2 className="h-4 w-4" />, color: "text-warning" },
};
...
export function CashTransactionList({
  ...
  const [typeFilter, setTypeFilter] = useState<CashTransaction["type"] | "all">("all");
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter((txn) => {
      const matchesSearch =
        ...
        (txn.relatedCustomerName?.toLowerCase().includes(search.toLowerCase()) ?? false);
    ...
    <div className={cn("space-y-4", className)}>
      ...
      <Card className="bg-gradient-to-br from-teal-50 to-cyan-50 border-teal-100">
        <CardBody className="text-center py-8">
          ...
          <p className="text-4xl font-bold text-teal-700">{formatCurrency(currentBalance)}</p>
        </CardBody>
      </Card>
          ...
          <CardBody className="py-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-semibold text-success">{formatCurrency(totals.cashIn)}</p>
              </div>
            </div>
          </CardBody>
        </Card>
          ...
          <CardBody className="py-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-semibold text-error">{formatCurrency(totals.cashOut)}</p>
              </div>
            </div>
          </CardBody>
        </Card>
      ...
      <div className="flex gap-3">
        ...
        <Select
          options={typeOptions}
          value={typeFilter}
          onChange={(value) => setTypeFilter(value as CashTransaction["type"] | "all")}
          className="w-48"
        />
      </div>
                ...
                <CardBody className="py-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={cn(
                        "p-2 rounded-lg",
                        txn.type === "in" ? "bg-success-light" :
                        txn.type === "out" ? "bg-error-light" :
                        "bg-warning-light"
                      )}>
                        {config.icon}
                      </div>
